				...
				enum iostat_type io_type)
{
	...
	trace_f2fsj_writepage(page, META);
				...
				struct writeback_control *wbc)
{
	...
	trace_f2fsj_writepages(mapping->host, wbc, META);
...
{
	trace_f2fsj_set_page_dirty(page, META);
...
int f2fs_sync_dirty_inodes(struct f2fs_sb_info *sbi, enum inode_type type)
{
	...
	trace_f2fsj_sync_dirty_inodes_enter(sbi->sb, is_dir,
				get_pages(sbi, is_dir ?
				F2FS_DIRTY_DENTS : F2FS_DIRTY_DATA));
retry:
	if (unlikely(f2fs_cp_error(sbi))) {
		trace_f2fsj_sync_dirty_inodes_exit(sbi->sb, is_dir,
				get_pages(sbi, is_dir ?
				F2FS_DIRTY_DENTS : F2FS_DIRTY_DATA));
		return -EIO;
	}
...
int f2fs_write_checkpoint(struct f2fs_sb_info *sbi, struct cp_control *cpc)
{
	...
	trace_f2fsj_write_checkpoint(sbi->sb, cpc->reason, "start block_ops");
	...
	trace_f2fsj_write_checkpoint(sbi->sb, cpc->reason, "finish block_ops");
...
stop:
	...
	trace_f2fsj_write_checkpoint(sbi->sb, cpc->reason, "finish checkpoint");
out:
	if (cpc->reason != CP_RESIZE)
		up_write(&sbi->cp_global_sem);
	return err;
...
int f2fs_issue_checkpoint(struct f2fs_sb_info *sbi)
{
	...
	if (cprc->f2fs_issue_ckpt)
    {
        ...
        INFO_REPORT("issuse and wait cp once\n\n");
    }
    else
    {
